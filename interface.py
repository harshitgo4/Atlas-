...
class hangman:
    ...
    word_list = ['afghanistan', 'argentina', 'australia', 'brazil', 'canada', 'chile', 'china', 'denmark', 'egypt',
                 'finland', 'germany', 'greece', 'hungary', 'india', 'ireland', 'jamaica', 'japan', 'kenya', 'malaysia',
                 'mexico', 'nigeria', 'pakistan', 'portugal', 'russia', 'spain', 'sweden', 'turkey', 'ukraine',
                 'uruguay', 'vietnam']
    ...
    def set_create_board(self, word):
        word_list_playing = [word[0]] + (['_'] * (len(word) - 1))
    ...
    def set_guess(self, player_guess):
        if (player_guess in self.gameboard_finished):  # Checking if guess is in found in gameboard_finished
    ...
    def get_eg_status(self):
        if (len(self.lives) == 6):
            ...
            messagebox.showinfo("GAME OVER!", "GAME OVER: Thanks for playing! \n Answer:\t" + str(
                ''.join([str(elem) for elem in self.gameboard_finished])))
            main_form.quit()
    ...
    def get_user_guess(self, letter):
        ...
        if len(char) == 1 and char.isalpha():
            if char.lower() in self.gameboard_finished:
                for position, char_in_word in enumerate(self.gameboard_finished):
                    if char.lower() == char_in_word:
                        self.set_move(self, char.lower(), position)
                        self.guess_archieve.append(char.lower())
            else:
                self.lives.append('x')
                self.guess_archieve.append(char.lower())
        else:
...
gui_gameboard = tk.Label(main_form, text=' '.join(game.gameboard), font="Verdana 30 bold")
...
def create_button(letter, xpos, ypos, index):
    letter = tk.Button(main_form, text=(alphaList[index].upper()),
                       command=lambda: btn_Click(letter, alphaList[index].upper()))
    letter.pack()
